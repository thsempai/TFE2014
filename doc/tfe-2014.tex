...
% De plus, nous avons abordé notre mode de fonctionnement de travail lors du developpement de l'application:

% Avant de commencer, pour chaque composant de l'application qui aura été identifé, une série de test préparés. Ces tests seront garant du bon fonctionnement du composant et donc de l'application.
% C'est test seront rédigé par le devellopeur (donc moi) conjointement avec le client afin d'être sûr de couvrir à la fois le coté technique et focntionnelle de l'application.

% Pendant la période de developement, nous ferons des scéances de tests, toutes les 3 semaines.
% Lors de ces scéances seront présent, trois testeurs (un par Systèmes d'exploitation) et le client.
